...
class AdminListView(View):
    ...
    async def get(self):
        async with self.request.app['db'].acquire() as conn:
            ...
            return render_template('admin/views/list.html',
                                   self.request,
                                   data)
...
class AdminAddView(View):
    ...
    async def get(self):
        ...
        return render_template('admin/views/add.html',
                               self.request,
                               result)
    ...
    async def post(self):
        ...
        if form.validate():
            async with self.request.app['db'].acquire() as conn:
                values = {key: form[key].data for key in form._fields.keys() if not key in [
                    'csrf_token', ]}
...
class AdminEditView(View):
    ...
    async def get(self):
        async with self.request.app['db'].acquire() as conn:
            ...
            item = await conn.fetchrow(
                self.model.select()
                    .where(self.model.c.id == typify(self.model.c.id, item_id)))
            data = await self.request.post()
            ...
            return render_template('admin/views/edit.html',
                                   self.request,
                                   result)
    ...
    async def post(self):
        ...
        if form.validate():
            async with self.request.app['db'].acquire() as conn:
                values = {key: form[key].data for key in form._fields.keys() if key not in [
                    'csrf_token', ]}
...
def admin_register(app, model, prefix='/admin/', primary='id', list_columns=None,
                   serarch_columns=None, edit_columns=None, meta=None):
    ...
    list_view = type(
        'Admin{}List'.format(model.name.capitalize()),
        (AdminListView, ),
        list_params
    )
    app.router.add_route('*', '{}{}'.format(prefix, name), list_view,
                         name=views['list'])

    add_views = copy.deepcopy(views)
    add_views['breadcrumb'].append(
        {'title': 'Добавить',
         'url': 'admin.{}.add'.format(name)}
    )
    ...
    add_view = type(
        'Admin{}Add'.format(model.name.capitalize()),
        (AdminAddView, ),
        add_params
    )
    app.router.add_route('*', '{}{}/add'.format(prefix, name), add_view,
                         name=views['add'])

    edit_views = copy.deepcopy(views)
    edit_views['breadcrumb'].append(
        {'title': 'Редактировать',
         'url': 'admin.{}.edit'.format(name)}
    )
    ...
    edit_view = type(
        'Admin{}Edit'.format(model.name.capitalize()),
        (AdminEditView, ),
        edit_params
    )
    app.router.add_route('*', '{}{}/{{id}}'.format(prefix, name), edit_view,
                         name=views['edit'])
...
async def admin_dashboard_view(request):
    ...
    async with request.app['db'].acquire() as conn:
        for view, model in request.app.get('admin'):
            item = await conn.fetchrow(
                select([func.count(model.c.id).label('count'), ])
            )
            elements.append({'view': view,
                             'count': item['count'],
                             'name': model.name})
...
def admin_dashboard(app, prefix='/admin/'):
    app.router.add_route(
        'GET', prefix, admin_dashboard_view, name='admin.dashboard')
